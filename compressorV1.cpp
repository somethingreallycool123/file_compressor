...
void decompressFile(const string& inputFile, const string& outputFile, Node* root) {
    ...
    int paddingBits = static_cast<int>(input.get());
    bitset<8> byte;
    char c;
    ...
    // Remove the padded bits from the decompressed data
    decompressedData.resize(decompressedData.length() - paddingBits);

    output << decompressedData;
}
...
int main() {
    ...
    // Decompress the file
    decompressFile(outputFile, decompressedFile, root);
